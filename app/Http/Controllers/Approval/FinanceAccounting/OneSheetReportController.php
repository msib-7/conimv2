...
class OneSheetReportController extends Controller
{
    public function index(Request $request)
    {
                ...
                ->addColumn('action', function ($row) {
                    $btn = '<ul class="list-inline text-center mb-0">
                                                <li class="list-inline-item align-bottom"
                                                    title="View">
                                                    <a href="' . route('v1.financeAccounting.osr.show', $row->id) . '"
        ...
        return view('v1.osr.financeAccount.index');
    }
    ...
    public function show($id)
    {
        ...
        return view('v1.osr.financeAccount.show', compact('data'));
    }
    ...
    private function berhasil($data, $request)
    {
        try {
            ...
            event(new NotificationEvent(
                ...
                'One Sheet Report telah disetujui oleh Finance Account ' . auth()->user()->fullname,
                route('v1.osr.index')
            ));
            ...
            return response()->json([
                ...
                'redirect' => route('v1.financeAccounting.osr.index')
            ]);
        } catch (\Throwable $th) {
            // throw $th;
            DB::rollBack();
    ...
    private function balikan($data, $request)
    {
        ...
        try {
            ...
            event(new NotificationEvent(
                ...
                'One Sheet Report telah return oleh Finance Accounting ' . auth()->user()->fullname,
                route('v1.osr.index')
            ));
