...
class CostSavingReportController extends Controller
{
    public function index(Request $request)
    {
                ...
                ->addColumn('action', function ($row) {
                    $btn = '<ul class="list-inline text-center mb-0">
                                                <li class="list-inline-item align-bottom"
                                                    title="View">
                                                    <a href="' . route('v1.mstdSpv.costSavingReport.show', $row->id) . '"
        ...
        return view('v1.csr.mstdSpv.index');
    }
    ...
    public function show($id)
    {
        ...
        return view('v1.csr.mstdSpv.show', compact('data', 'jenis_saving'));
    }
        ...
        try {
            ...
            if (intval($data->cost_saving) > 30000000) {
                ...
                event(new NotificationEvent(
                    ...
                    'Cost Saving Report telah disetujui oleh MSTD SPV ' . auth()->user()->fullname,
                    route('v1.csr.index')
                ));
                ...
                event(new NotificationEvent(
                    $data->user_id,
                    'Finished Approval',
                    'Cost Saving Report telah Selesai dan silahkan cek kembali',
                    route('v1.csr.index')
                ));

                $message = 'Halo, Persetujuan anda sudah selesai untuk Cost Saving Report, Silahkan Cek Terlebih dahulu';
                $email = [
                    'status' => 'Waiting Approval Finance Accounting',
                    'link' => route('v1.financeAccounting.costSavingReport.index'),
                ...
                $data = [
                    'status' => 'Approval FA',
                    'link' => route('v1.financeAccounting.costSavingReport.index'),
                    'penerima' => 'Finance Accouting',
                    'body' => $message
                ];
                $mstdOfficer = UserFa::pluck('email');
            ...
            return response()->json([
                ...
                'redirect' => route('v1.mstdSpv.costSavingReport.index')
            ]);
        } catch (\Throwable $th) {
            // throw $th;
            DB::rollBack();
    ...
    private function balikan($data, $request)
    {
        try {
            ...
            event(new NotificationEvent(
                ...
                'Cost Saving Report telah return oleh MSTD SPV ' . auth()->user()->fullname,
                route('v1.csr.index')
            ));

            $message = 'Halo, Persetujuan anda Cosr Saving Report telah dikembalikan dengan catatan : ' . $request->note . ' , Silahkan Cek Terlebih dahulu';
            $email = [
